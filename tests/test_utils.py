...
file = 'test_operations.json'
count = 1


def test_load_file():
    assert load_file('test_operations1.json') is None
    assert len(load_file(file)) > 0
    assert isinstance(load_file(file), list)
    assert load_file('test_operations.json') == [
        {
            "id": 441945886,
            "state": "EXECUTED",
...
def test_executed_last_operations():
    data = load_file(file)
    assert executed_last_operations(data, count) == [{'date': '2019-10-30T01:49:52.939296',
    ...
    assert executed_last_operations(data) == [{'date': '2019-10-30T01:49:52.939296',
                                               'description': 'Перевод с карты на счет',
                                               'from': 'Visa Gold 7756673469642839',
                                               'id': 509645757,
                                               'operationAmount': {'amount': '23036.03',
